...
resolve_asset_filename() {
  ...
  then
    EXT='.zip'
    BINARY_NAME_0=cert-prep
    BINARY_PATH_0=cert-prep.exe
  fi
  if [ -z "${ASSET_FILENAME}" ]; then
    ASSET_FILENAME="cert-prep-${OS}-${ARCH}${EXT}"
  fi
}
...
execute() {
  ...
  BINARY_NAME='cert-prep'
  if [ -z "${EXT}" ] || [ "${EXT}" = ".exe" ]; then
    BINARY_PATH="${TMPDIR}/${ASSET_FILENAME}"
  else
    BINARY_PATH="${TMPDIR}/cert-prep"
  fi
  if [ -n "$BINARY_NAME_0" ]; then
    BINARY_NAME="$BINARY_NAME_0"
  fi
  if [ -n "$BINARY_PATH_0" ]; then
    BINARY_PATH="${TMPDIR}/$BINARY_PATH_0"
  fi
